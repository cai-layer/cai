...
- **`.caiShowToast` carries action OUTPUT, not just Cai's own messages** — `ChainExecutor`'s success path posts up to 60 chars of the action's result through it. `WindowController`'s observer runs TCC-denial detection over every message, so a result that merely *mentions* a denial would get its toast swallowed and a permission prompt raised instead. Result-bearing toasts carry `isActionResult: true` and skip that hook; respect the flag if you add anything else that inspects toast text.
- **Never gate a permission escape hatch on an authorization status read** — `authorizationStatus(for:)` caches per process (measured: a fresh process read `notDetermined` while a long-running one read `.authorized` at the same instant), and an `EKEventStore`/`CNContactStore` instance that has already seen an answer short-circuits `requestFullAccess*` without contacting tccd at all. So stores are recreated per request, and status may pick a button's *wording* but never whether the button exists. Both traps look exactly like a missing entitlement. Full detail + the `codesign` check that tells them apart: [`_docs/architecture/PERMISSIONS.md`](_docs/architecture/PERMISSIONS.md).
